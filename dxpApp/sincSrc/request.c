...
{
    // Send it.
    int errCode = SincSocketWrite(sc->fd, sendBuf->data, (int)sendBuf->len);
    PROTOBUF_C_BUFFER_SIMPLE_CLEAR(sendBuf);
    if (errCode != 0)
    {
...
bool SincRequestGetParams(Sinc *sc, int *channelIds, const char **names, int numNames)
{
    ...
    uint8_t pad[256];
    SincBuffer sendBuf = SINC_BUFFER_INIT(pad);
    SincEncodeGetParams(&sendBuf, channelIds, names, numNames);
...
bool SincRequestSetParams(Sinc *sc, int channelId, SiToro__Sinc__KeyValue *params, int numParams)
{
    ...
    uint8_t pad[256];
    SincBuffer sendBuf = SINC_BUFFER_INIT(pad);
    SincEncodeSetParams(&sendBuf, channelId, params, numParams);
...
bool SincRequestSoftwareUpdate(Sinc *sc, uint8_t *appImage, int appImageLen, char *appChecksum, uint8_t *fpgaImage, int fpgaImageLen, char *fpgaChecksum, SincSoftwareUpdateFile *updateFiles, int updateFilesLen, int autoRestart)
{
    ...
    uint8_t pad[256];
    SincBuffer sendBuf = SINC_BUFFER_INIT(pad);
    SincEncodeSoftwareUpdate(&sendBuf, appImage, appImageLen, appChecksum, fpgaImage, fpgaImageLen, fpgaChecksum, updateFiles, updateFilesLen, autoRestart);
...
bool SincRequestSaveConfiguration(Sinc *sc, int channelId)
{
    ...
    uint8_t pad[256];
    SincBuffer sendBuf = SINC_BUFFER_INIT(pad);
    SincEncodeSaveConfiguration(&sendBuf, channelId);
...
bool SincRequestMonitorChannels(Sinc *sc, int *channelSet, int numChannels)
{
    ...
    uint8_t pad[256];
    SincBuffer sendBuf = SINC_BUFFER_INIT(pad);
    SincEncodeMonitorChannels(&sendBuf, channelSet, numChannels);
